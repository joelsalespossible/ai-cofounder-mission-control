...
Env vars:
  ACP_BRIDGE_TOKEN  — auth token (required, no default)
  ACP_BRIDGE_PORT   — listen port (default 18790)
  ACP_BRIDGE_DIR    — data directory (default /tmp/acp_bridge)

Optional: set GATEWAY_HOOKS_URL to forward inbox messages to a gateway
webhook (fire-and-forget wake signal).
"""
import json, os, time, fcntl, urllib.request
...
GATEWAY_HOOKS_URL = os.environ.get("GATEWAY_HOOKS_URL", "")
GATEWAY_TOKEN = os.environ.get("GATEWAY_HOOKS_TOKEN", "")

if not AUTH_TOKEN:
    print("WARNING: ACP_BRIDGE_TOKEN not set. Generate one: openssl rand -hex 16", flush=True)
...
def forward_to_gateway(text):
    """Optional: fire-and-forget POST to gateway hooks to wake the receiving agent."""
    if not GATEWAY_HOOKS_URL:
        return
    try:
        payload = json.dumps({"text": f"[BRIDGE] {text[:200]}"}).encode()
        headers = {"Content-Type": "application/json"}
        if GATEWAY_TOKEN:
            headers["Authorization"] = f"Bearer {GATEWAY_TOKEN}"
        req = urllib.request.Request(GATEWAY_HOOKS_URL, data=payload, headers=headers, method="POST")
